    ...
    fn steal(&self, index: usize, xorshift: &mut usize) -> Option<NonNull<Task>> {
        for _ in 0..1 {
            ...
            *xorshift = rng;
            ...
            let mut was_contended = match self.workers[index].buffer.consume(&self.workers[index].queue) {
            ...
            {
                was_contended = match self.workers[index].buffer.consume(&self.workers[steal_index].queue) {
                    Ok(task) => return Some(task),
