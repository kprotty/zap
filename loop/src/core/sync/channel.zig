...
pub fn Channel(comptime AutoResetEvent: type, comptime T: type) type {
    return struct {
        const Self = @This();
        const Lock = GenericLock(AutoResetEvent);
        //const Lock = GenericLock(@import("../../system/system.zig").AutoResetEvent);
