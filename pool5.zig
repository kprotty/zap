...
const Buffer = struct {
    ...
    fn popAndStealList(noalias self: *Buffer, noalias target: *List) ?Popped {
        var consumer = target.getConsumer();
...
const List = struct {
    head: Atomic(?*Runnable) = Atomic(?*Runnable).init(null),
    tail: Atomic(?*Runnable) align(cache_line_padding) = Atomic(?*Runnable).init(null),

    fn push(self: *List, batch: Batch) void {
        const head = batch.head orelse return;
        const tail = batch.tail orelse unreachable;

        const prev = self.tail.swap(tail, .Monotonic);
        const next = if (prev) |p| @ptrCast(*Atomic(?*Runnable), &p.next) else &self.head;
        next.store(head, .Release);
    }

    fn getConsumer(self: *List) Consumer {
        return .{ .list = self };
    }
    ...
    const Consumer = struct {
        ...
        fn pop(self: *Consumer) ?*Runnable {
            const head = self.head orelse return self.consume();
            return self.popFast(head) orelse self.popSlow(head);
        }

        fn popFast(noalias self: *Consumer, noalias head: *Runnable) ?*Runnable {
            const next_ptr = @ptrCast(*Atomic(?*Runnable), &head.next);
            self.head = next_ptr.load(.Acquire) orelse return null;
            return head;
        }

        fn popSlow(noalias self: *Consumer, noalias head: *Runnable) ?*Runnable {
            @setCold(true);

            var spin: u8 = 10;
            while (spin > 0) : (spin -= 1) {
                std.atomic.spinLoopHint();
                if (self.popFast(head)) |runnable| {
                    return runnable;
                }

                const tail = self.list.tail.load(.Monotonic) orelse unreachable;
                if (tail != head) {
                    continue;
                }

                _ = self.list.tail.tryCompareAndSwap(
                    tail,
                    null,
                    .Acquire,
                    .Monotonic,
        ...
        fn consume(self: *Consumer) ?*Runnable {
            @setCold(true);

            var head = self.list.head.load(.Monotonic) orelse return null;
            head = consumed: {
                if (comptime arch.isX86()) {
                    const old_head = self.list.head.swap(null, .Acquire);
                    break :consumed (old_head orelse return null);
                }

                while (true) {
                    const old_head = self.list.head.tryCompareAndSwap(
                        head,
                        null,
                        .Acquire,
                        .Monotonic,
                    ) orelse break :consumed head;
                    head = old_head orelse return null;
                }
            };
        ...
        fn release(self: Consumer) void {
            ...
            self.list.head.store(head, .Release);
        }
    };
};
