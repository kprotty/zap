...
const StackPool = struct {
    workers: []Worker,
    state: usize = 0,
    idle_mutex: Mutex = Mutex.init(),
    idle_cond: Condvar = Condvar.init(),
    
    const Self = @This();
    const IS_SHUTDOWN = std.math.maxInt(usize);
    const IS_WAITING: usize = 1 << 1;
    const IS_NOTIFIED: usize = 1 << 0;
    ...
    fn run(runnable: *Runnable) !void {
        const allocator = std.heap.page_allocator;
        const num_threads = std.Thread.cpuCount() catch 1;
        const workers = try allocator.alloc(Worker, num_threads);
        defer allocator.free(workers);

        var pool = Self{ .workers = workers };
        for (workers) |*w| 
            w.* = .{ .pool = &pool };
        ...
        defer {
            pool.exit();
            for (workers) |*w| {
                const thread = w.thread orelse continue;
                thread.wait();
            }
        ...
        workers[0].queue.push(runnable);
        for (workers[1..]) |*w| 
            w.thread = try std.Thread.spawn(w, Worker.run);
        workers[0].run();
    }

    fn schedule(runnable: *Runnable) void {
        const worker = @intToPtr(?*Worker, TLS.get()) orelse unreachable;
        worker.queue.push(runnable);
        worker.pool.notify();
    }
    ...
    fn exit(self: *Self) void {
        const state = @atomicRmw(usize, &self.state, .Xchg, IS_SHUTDOWN, .AcqRel);
    ...
    fn wake(self: *Self, wake_all: bool) void {
    ...
    fn notify(self: *Self) void {
        ...
        while (true) {
            ...
            // std.debug.warn("notify({}) waiting={}\n", .{std.Thread.getCurrentId(), state >> 1});
    ...
    fn wait(self: *Self) ?bool {
        ...
        while (true) {
            ...
            if ((state & IS_NOTIFIED != 0) or !is_waiting) {
                var new_state = state & ~IS_NOTIFIED;
                if (state & IS_NOTIFIED != 0) {
                    if (is_waiting)
                        new_state -= IS_WAITING;
                } else if (!is_waiting) {
                    new_state += IS_WAITING;
                }
                ...
                // std.debug.warn("wait({}) s={} n={} waiting={} wake={}\n", .{std.Thread.getCurrentId(), state >> 1, new_state >> 1, is_waiting, state & IS_NOTIFIED != 0});
            ...
            while (true) {
                ...
                if (state == IS_SHUTDOWN or state & IS_NOTIFIED != 0) {
                    break;
                } else {
                    self.idle_cond.wait(&self.idle_mutex);
                }
            }
    ...
    const UnboundedQueue = struct {
        ...
        fn push(self: *UnboundedQueue, runnable: *Runnable) void {
            self.mutex.lock();
            ...
            @atomicStore(usize, &self.size, size + 1, .Monotonic);
        }

        fn tryAcquireConsumer(self: *UnboundedQueue) ?Consumer {
            if (@atomicLoad(usize, &self.size, .Monotonic) == 0)
                return null;

            if (!self.mutex.tryLock())
                return null;

            const size = @atomicLoad(usize, &self.size, .Monotonic);
            if (size == 0) {
                self.mutex.unlock();
                return null;
            }
        ...
        const Consumer = struct {
            ...
            fn release(self: Consumer) void {
                @atomicStore(usize, &self.queue.size, self.size, .Monotonic);
                self.queue.mutex.unlock();
            }

            fn pop(self: *Consumer) ?*Runnable {
                const runnable = self.queue.head orelse return null;
                self.queue.head = runnable.next;
    ...
    const RunQueue = struct {
        head: usize = 0,
        tail: usize = 0,
        buffer: [128]*Runnable = undefined,
        overflowed: UnboundedQueue = .{},
        
        fn deinit(self: *RunQueue) void {
            self.overflowed.deinit();
            self.* = undefined;
        }
        ...
        fn push(self: *RunQueue, runnable: *Runnable) void {
            ...
            if (tail -% head < self.buffer.len) {
                ...
                self.overflowed.push(runnable);
            }
        }

        fn pop(self: *RunQueue) ?*Runnable {
            const tail = self.tail -% 1;
            @atomicStore(usize, &self.tail, tail, .SeqCst);
            const head = @atomicLoad(usize, &self.head, .SeqCst);

            var runnable: ?*Runnable = null;
            if (tail <= head) {
                runnable = self.buffer[tail % self.buffer.len];
                if (tail != head)
                    return runnable;
                if (@cmpxchgStrong(
                    usize,
            ...
            @atomicStore(usize, &self.tail, tail +% 1, .Monotonic);
            return runnable orelse self.stealOverflow(self);
        }

        fn stealOverflow(self: *RunQueue, target: *RunQueue) ?*Runnable {
            var consumer = target.overflowed.tryAcquireConsumer() orelse return null;
            defer consumer.release();
            ...
            const tail = self.tail;
            var pushed: usize = 0;
            while (pushed < self.buffer.len) {
                const r = consumer.pop() orelse break;
                @atomicStore(*Runnable, &self.buffer[(tail +% pushed) % self.buffer.len], r, .Unordered);
                pushed +%= 1;
            }

            if (pushed > 0)
                @atomicStore(usize, &self.tail, tail +% pushed, .Release);
            return first;
        }

        fn steal(self: *RunQueue, target: *RunQueue) ?*Runnable {
            if (target == self)
                return self.pop();
            ...
            var target_head = @atomicLoad(usize, &target.head, .SeqCst);
            while (true) {
                const target_tail = @atomicLoad(usize, &target.tail, .SeqCst);
                if (target_head >= target_tail)
                    return self.stealOverflow(target);

                const r = @atomicLoad(*Runnable, &target.buffer[target_head % target.buffer.len], .Unordered);
    ...
    const RunQueue2 = struct {
        ...
        fn push(self: *@This(), runnable: *Runnable) void {
            self.mutex.lock();
            defer self.mutex.unlock();

            runnable.next = self.top;
            @atomicStore(?*Runnable, &self.top, runnable, .Monotonic);
        }

        fn pop(self: *@This()) ?*Runnable {
            return self.steal(self);
        }

        fn steal(self: *@This(), target: *@This()) ?*Runnable {
            if (@atomicLoad(?*Runnable, &self.top, .Monotonic) == null)
                return null;

            if (!self.mutex.tryLock()) return null;
            defer self.mutex.unlock();

            const runnable = self.top orelse return null;
            @atomicStore(?*Runnable, &self.top, runnable.next, .Monotonic);
            return runnable;
        }
    };
    ...
    const Worker = struct {
        ...
        fn run(self: *Worker) void {
            ...
            var prng = @truncate(u32, @ptrToInt(self) >> @sizeOf(usize));

            while (true) {
                if (self.poll(&prng)) |runnable| {
                    (runnable.callback)(runnable);
                } else {
                    _ = self.pool.wait() orelse break;
                }
            }
        }

        fn poll(self: *Worker, prng: *u32) ?*Runnable {
            if (self.queue.pop()) |runnable|
                return runnable;

            const num_workers = self.pool.workers.len;
            var steal_attempts: u8 = 4;
            while (steal_attempts > 0) : (steal_attempts -= 1) {
                var index = blk: {
                    ...
                    x ^= x << 5;
                    prng.* = x;
                    break :blk (@as(usize, x) % num_workers);
                };

                var iter = num_workers;
                while (iter > 0) : (iter -= 1) {
                    const target = &self.pool.workers[index];
                    index += 1;
                    if (index >= num_workers)
                        index = 0;
                    if (target == self)
                        continue;
                    if (self.queue.steal(&target.queue)) |runnable|
                        return runnable;
                }
            }
