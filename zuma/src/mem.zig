...
var cached_page_size = zync.utils.Lazy(backend.pageSize).new();
...
var cached_huge_page_size = zync.utils.Lazy(backend.hugePageSize).new();
...
pub fn map(handle: ?zio.Handle, address: ?*align(page_size) const u8, bytes: usize, flags: u32) MapError![]align(page_size) u8 {
    return backend.map(handle, address, bytes, flags);
}

pub fn unmap(memory: []align(page_size) u8) void {
...
pub fn modify(memory: []align(page_size) u8, flags: u32) ModifyError!void {
